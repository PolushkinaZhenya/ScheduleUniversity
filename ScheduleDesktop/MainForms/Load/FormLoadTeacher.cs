...
namespace ScheduleDesktop
{
	public partial class FormLoadTeacher : Form
	{
		...
		private readonly IBaseService<FlowBindingModel, FlowViewModel, FlowSearchModel> serviceF;
		...
		public FormLoadTeacher(ILoadTeacherService service,
			...
			IBaseService<TeacherBindingModel, TeacherViewModel, TeacherSearchModel> serviceT,
			IBaseService<FlowBindingModel, FlowViewModel, FlowSearchModel> serviceF,
		...
		private void FormLoadTeacher_Load(object sender, EventArgs e)
		{
			try
			{
				...
				List<FlowViewModel> listF = serviceF.GetList(new FlowSearchModel
				{
					FlowAutoCreation = false,
					StudentGroupId = serviceSG.GetElement(new StudyGroupSearchModel { Title = studyGroupTitle }).Id
				});
				if (listF != null)
				{
		...
		{
			var form = DependencyManager.Instance.Resolve<FormLoadTeacherPeriod>();
		...
		private void ButtonAddAuditorium_Click(object sender, EventArgs e)
		{
			...
			{
				var form = DependencyManager.Instance.Resolve<FormLoadTeacherAuditorium>();
				if (form.ShowDialog() == DialogResult.OK)
				{
					if (form.Model != null)
					{
		...
		private void Save()
		{
			...
			{
				var flows = serviceF.GetList();// все потоки базы
				...
				{
					var flowStudyGroup = flows[i].FlowStudyGroups[0];//с чем сравнивать

					if (flowStudyGroup.StudyGroupTitle == studyGroupTitle && flowStudyGroup.Subgroup.ToString() == textBoxNumberOfSubgroups.Text)
					{
						//если поток есть
						flow = serviceF.GetElement(new FlowSearchModel { Id = flows[i].Id });
						flowexists = true;
					}
				}
				if (!flowexists)
				{
					...
					var model = new FlowStudyGroupViewModel
					{
						...
						Subgroup = textBoxNumberOfSubgroups.Text == "" ? (int?)null : int.Parse(textBoxNumberOfSubgroups.Text)
					};
					...
					{
						Title = studyGroupTitle + (textBoxNumberOfSubgroups.Text == "" ? null : " п/г-" + int.Parse(textBoxNumberOfSubgroups.Text)),
						FlowAutoCreation = true,
						FlowStudyGroups = FlowStudyGroupBM
					});
					//записываем новый поток
					flow = serviceF.GetElement(new FlowSearchModel
					{ Title = studyGroupTitle + (textBoxNumberOfSubgroups.Text == "" ? null : " п/г-" + int.Parse(textBoxNumberOfSubgroups.Text)) });
				}
			}
			else
			{
				flow = serviceF.GetElement(new FlowSearchModel { Id = (Guid)comboBoxFlow.SelectedValue });
			}

			var LoadTeacherPeriodBM = new List<LoadTeacherPeriodBindingModel>();
			for (int i = 0; i < LoadTeacherPeriods.Count; ++i)
			{
			...
			var LoadTeacherAuditoriumBM = new List<LoadTeacherAuditoriumBindingModel>();
			for (int i = 0; i < LoadTeacherAuditoriums.Count; ++i)
			{
			...
			if (id.HasValue)
			{
				...
				service.UpdElement(new LoadTeacherBindingModel
				{
					...
					NumberOfSubgroups = textBoxNumberOfSubgroups.Text == "" ? (int?)null : int.Parse(textBoxNumberOfSubgroups.Text)
				});
			}
			else
			{
		...
		private void AddSchedule(Guid LoadTeacherId)
		{
			try
			{
				...
				foreach (var LoadTeacherPeriod in LoadTeacherPeriods)
				{
					...
					listFlowStudyGroup = serviceF.GetElement(new FlowSearchModel { Id = loadTeacher.FlowId }).FlowStudyGroups;
			...
			List<LoadTeacherAuditoriumViewModel> LoadTeacherAuditoriumNew)
		{
			...
			var LoadTeacherPeriodUpd = new List<LoadTeacherPeriodViewModel>();
			var LoadTeacherAuditoriumUpd = new List<LoadTeacherAuditoriumViewModel>();
		...
		private void ButtonAddFlow_Click(object sender, EventArgs e)
		{
			...
			if (form.ShowDialog() == DialogResult.OK)
			{
				List<FlowViewModel> listF = serviceF.GetList(new FlowSearchModel { FlowAutoCreation = false });
				if (listF != null)
				{
					comboBoxFlow.DisplayMember = "Title";
					comboBoxFlow.ValueMember = "Id";
